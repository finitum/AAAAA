<template>
    <div class="flex flex-row items-center justify-between w-100">
        <label for="d">
            <span>days:</span>
            <input id="d" type="number" v-model="day" @change="update()" min=0 @focus="$event.target.select()" required/>
        </label>
        <label for="h">
            <span>hours:</span>
            <input id="h" type="number" v-model="hour" @change="update()" min=0 @focus="$event.target.select()" required/>
        </label>
        <label for="m">
            <span>minutes:</span>
            <input id="m" type="number" v-model="min" @change="update()" min=0 @focus="$event.target.select()" required/>
        </label>
    </div>
</template>
...
<script lang="ts">
    ...
    export default defineComponent({
        ...
        setup(props, {emit}) {
            ...
            watch(() => props.modelValue, (value, prev) => {
